...
# A node's children all live in one extents vector at the next level.
# Resolve that vector once per node and index into it, instead of following
# `parent_index -> levels -> level -> extents` again for every child.
