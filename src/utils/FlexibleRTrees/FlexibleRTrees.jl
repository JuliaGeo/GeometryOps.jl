...
dimensionality, hosting interchangeable bulk-load algorithms — sort-tile-
recursive ([`STR`](@ref)), Hilbert-packed ([`HPR`](@ref)), or none
([`Unsorted`](@ref)) — behind one tree type.

The initial construction logic is adapted from
[SortTileRecursiveTree.jl](https://github.com/maxfreu/SortTileRecursiveTree.jl)
(MIT), with two structural changes:

1. **Flat, depth-invariant storage.**  SortTileRecursiveTree.jl nests
   `STRNode{E, Vector{STRNode{E, …}}}`, so the *type* of the tree depends on
   its depth — a fresh specialization per depth and a type-unstable root.
   Here the tree is a vector of per-level extent vectors plus a leaf
   permutation (the layout of `NaturalIndexing.NaturalIndex` and of JTS's
   `HPRtree`): `RTree{A, E}` is concrete regardless of size or depth.
2. **The bulk-load algorithm is a value.**  All packing is "order the leaves,
   then union consecutive runs of `nodecapacity` upward"; algorithms differ
   only in the leaf ordering they produce via [`loadorder`](@ref).  Adding an
   algorithm is one singleton type and one `loadorder` method.

Note one behavioral difference this layout buys: upper levels group *runs of
the leaf order* rather than re-tiling each level the way pointer-based STR
does.  Hilbert order has locality at every scale, so `HPR` loses nothing;
`STR`'s upper levels are slightly looser than a re-tiled pointer tree.

The tree implements SpatialTreeInterface, so `depth_first_search` /
`dual_depth_first_search` (and everything built on them, like prepared
geometry's point-in-polygon) work unchanged.  Leaf queries yield indices into
...
function RTree(algorithm::BulkLoadAlgorithm, data; nodecapacity::Int = 16)
    ...
    return RTree{typeof(algorithm), E}(algorithm, nodecapacity, total, levels, perm)
end
...
#
# Same cursor scheme as `NaturalIndexing.NaturalIndexNode`, including the
# hoisted child-extents vector (resolved once per node, not once per child).
# The one addition: leaf queries map slots through `tree.indices` so callers
# get indices into the original collection despite the packed reordering.
