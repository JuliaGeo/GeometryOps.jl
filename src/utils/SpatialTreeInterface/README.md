...
- `node_extent_is_expensive(node)::Bool` - whether `node_extent` *computes* the node's extent rather than reading a stored one.  Defaults to `false`, which is right for any tree that stores an extent per node.  Trees that derive extents on access (a DGGS cursor fitting a spherical cap over cell geometry, say) should define it to be `true`, which lets `dual_depth_first_search` derive each child's extent once per node pair instead of once per opposing child.
...
These are both completely non-allocating, and will only call `f` for indices that satisfy the predicate.  (The dual query allocates a small vector per visited internal node only for trees that opt into `node_extent_is_expensive`, which is a deliberate trade of allocation for far fewer `node_extent` calls.)
