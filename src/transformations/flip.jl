...
function flip(geom; kw...)
    if _ismeasured(geom)
        return apply(PointTrait(), geom; kw...) do p
            SVPoint_4D(StaticArrays.SVector{4}((GI.y(p), GI.x(p), GI.z(p), GI.m(p))))
        end
    elseif _is3d(geom)
        return apply(PointTrait(), geom; kw...) do p
            SVPoint_3D(StaticArrays.SVector{3}((GI.y(p), GI.x(p), GI.z(p))))
        end
    else
        return apply(PointTrait(), geom; kw...) do p
            SVPoint_2D(StaticArrays.SVector{2}((GI.y(p), GI.x(p))))
        end
    end
end
