...
function mean_value_barycentric_coordinates(polypoints::Vector{<: Point{N1, T1}}, point::Point{N2, T2}) where {N1, N2, T1 <: Real, T2 <: Real}
    ...
    for i in 1:n
        @timeit to "Polygon access" begin
        prev = poly[mod1(i-2, n)]
        curr = poly[i]
        next = poly[mod1(i, n)]
        end
        @timeit to "Weight computation" begin
...
f, a, p = tricontourf(getindex.(polygon_points, 1), getindex.(polygon_points, 2), getindex.(polygon_points, 3), colormap = cgrad(:jet, 18; categorical = true),)
...
Colorbar(f[1, 2], p)
f
# poly!(polygon_points; color = last.(polygon_points), colormap = cgrad(:jet, 18; categorical = true), shading = false, strokewidth = 0.0)
f