...
@test_implementations GO.coverage($pt1, cell_extremes...) == 0.0
@test_implementations GO.coverage($mpt1, cell_extremes...) == 0.0
@test_implementations GO.coverage($l1, cell_extremes...) == 0.0
...
@test_implementations GO.coverage($p1, cell_extent) == cell_area
# polygon is the same as the cell (input is min/max values)
@test_implementations GO.coverage($p1, cell_extremes...) == cell_area
# polygon is bigger than the cell
p2 = GI.Polygon([[(-10, -10.0), (-10.0, 30.0), (30.0, 30.0), (300.0, -10.0), (-10.0, -10.0)]])
@test_implementations GO.coverage($p2, cell_extremes...) == cell_area
...
@test_implementations GO.coverage($p10, cell_extremes...) ≈ LG.area(LG.intersection(p10, cell_poly))
...
@test_implementations GO.coverage($p3, cell_extremes...) ≈ LG.area(LG.intersection(cell_poly, p3))
# polygon exits cell through one edge
@test_implementations GO.coverage($p4, cell_extremes...) ≈ LG.area(LG.intersection(cell_poly, p4))
@test_implementations GO.coverage($p5, cell_extremes...) ≈ LG.area(LG.intersection(cell_poly, p5))
# polygon exits cell through multiple edges (north and east)
@test_implementations GO.coverage($p6, cell_extremes...) ≈ LG.area(LG.intersection(p6, cell_poly))
# polygon exits cell through multiple edges (west and east)
@test_implementations GO.coverage($p7, cell_extremes...) ≈ LG.area(LG.intersection(p7, cell_poly))
# non-convex polygon split into two pieces (same wall)
@test_implementations GO.coverage($p8, cell_extremes...) ≈ LG.area(LG.intersection(p8, cell_poly))
# counter-clockwise polygon
@test_implementations GO.coverage($p9, cell_extremes...) ≈ LG.area(LG.intersection(p9, cell_poly))
# polygon with a hole
@test_implementations GO.coverage($p10, cell_extremes...) ≈ LG.area(LG.intersection(p10, cell_poly))
# non-convex polygon split into two pieces (differnet walls)
@test_implementations GO.coverage($p11, cell_extremes...) ≈ LG.area(LG.intersection(p11, cell_poly))
